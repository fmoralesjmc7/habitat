...
} from './producto.constantes';
import {
  DatosPrudential,
  DetalleSaldosConsolidadosResp,
} from '../interfaces/prudential';
...
export class LlamadaSaldosConsolidados {
  ...
  public obtenerDatosPrudential(): Promise<DatosPrudential> {
    return new Promise<any>(async (resolve, reject) => {
      const respuesta: DatosPrudential = {
        estadoConsolidacion: undefined,
      ...
      try {
        ...
        if (respuesta.preferencia) {
          respuesta.productos = (
            await this.obtenerSaldosConsolidados()
          ).detalleCuentas;
        }
  ...
  registrarDatosPrudential(prudential: DatosPrudential): void {
    if (prudential?.estadoConsolidacion) {
      this.prudentialDatos.setEstadoConsolidacion(
        prudential.estadoConsolidacion
      );
  ...
  private obtenerEstadoConsolidacion(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      this.prudentialService.obtenerEstadoConsolidacion().subscribe(
        (response) => {
  ...
  private obtenerPreferenciasSaldos(): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      this.prudentialService.obtenerPreferenciasSaldos().subscribe(
        (response) => {
  ...
  private obtenerSaldosConsolidados(): Promise<DetalleSaldosConsolidadosResp> {
    return new Promise<DetalleSaldosConsolidadosResp>((resolve, reject) => {
      this.prudentialService.obtenerSaldosConsolidados().subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
