...
import {
  DatosPrudential,
  SucripcionPrudentialRequest,
} from 'src/app/interfaces/prudential';
...
export class ModalPrudentialAutorizaConsolidacionComponent {
  ...
  async guardarConsolidacion(): Promise<void> {
    ...
    this.prudentialService.guardarSuscripcionPrudential(suscripcion).subscribe(
      async (response) => {
        const prudential =
          await this.llamadaSaldosConsolidados.obtenerDatosPrudential();
