...
export class DatosActualizarUsuario {
  ...
  public esSuscripcionCorreoComercial = false;
  public esSuscripcionCorreoParticular = false;
  public opcionResidenciaChile = false;

  public idRenta = 0;
  public nombreRenta = '';
  public idProfesion = 0;
  ...
  public iniciarDatos(datosUsuarioResponse: any, rut: number, dv: string) {
    ...
    this.dv = dv;
    this.nombres =
      datosUsuarioResponse.primer_nombre +
      ' ' +
      datosUsuarioResponse.segundo_nombre;
    ...
    this.esSuscripcionCorreoParticular =
      datosUsuarioResponse.tipo_suscripcion_cartola ===
      this.CONSTANTES.TIPO_SUSC_CORREO_PART
        ? true
        : false;
    this.esSuscripcionCorreoComercial =
      datosUsuarioResponse.tipo_suscripcion_cartola ===
      this.CONSTANTES.TIPO_SUSC_CORREO_COM
        ? true
        : false;
  ...
  public calculoNacionalidadUsuario(listadoNacionalidad: any) {
    let nacionalidad = listadoNacionalidad.find(
      (nacion: any) => nacion.id_nacionalidad === this.idNacionalidadParticular
    );
    if (nacionalidad) {
  ...
  obtenerCelularUsuario(listadoTelefonos: any) {
    this.celularUsuario = listadoTelefonos.find(
      (telefono: any) =>
        telefono.id_tipo_telefono === this.CONSTANTES.ID_TELEFONO_CELULAR
    );
    if (this.celularUsuario === undefined) {
      this.celularUsuario = Object.assign(
        {},
        this.CONSTANTES.OBJETO_TELEFONO_VACIO
      );
      this.celularUsuario.id_tipo_telefono =
        this.CONSTANTES.ID_TELEFONO_CELULAR;
    }
  }

  obtenerTelefonoParticular(listadoTelefonos: any) {
    this.telefonoParticular = listadoTelefonos.find(
      (telefono: any) =>
        telefono.id_tipo_telefono === this.CONSTANTES.ID_TELEFONO_PART
    );
    if (this.telefonoParticular === undefined) {
      this.telefonoParticular = Object.assign(
        {},
        this.CONSTANTES.OBJETO_TELEFONO_VACIO
      );
      this.telefonoParticular.id_tipo_telefono =
        this.CONSTANTES.ID_TELEFONO_PART;
    }
  }

  obtenerTelefonoComercial(listadoTelefonos: any) {
    this.telefonoComercial = listadoTelefonos.find(
      (telefono: any) =>
        telefono.id_tipo_telefono === this.CONSTANTES.ID_TELEFONO_COM
    );
    if (this.telefonoComercial === undefined) {
      this.telefonoComercial = Object.assign(
        {},
        this.CONSTANTES.OBJETO_TELEFONO_VACIO
      );
  ...
  obtenerCorreoParticular(listadoCorreos: any) {
    this.correoParticular = listadoCorreos.find(
      (correo: any) => correo.id_tipo_correo === this.CONSTANTES.ID_CORREO_PART
    );
    if (this.correoParticular === undefined) {
      this.correoParticular = Object.assign(
        {},
        this.CONSTANTES.OBJETO_CORREO_VACIO
      );
  ...
  obtenerCorreoComercial(listadoCorreos: any) {
    this.correoComercial = listadoCorreos.find(
      (correo: any) => correo.id_tipo_correo === this.CONSTANTES.ID_CORREO_COM
    );
    if (this.correoComercial === undefined) {
      this.correoComercial = Object.assign(
        {},
        this.CONSTANTES.OBJETO_CORREO_VACIO
      );
  ...
  obtenerTipoResidencia(listadoDirecciones: any) {
    let direccionNoResidente = listadoDirecciones.find(
      (direccion: any) =>
        direccion.id_prioridad === this.CONSTANTES.ID_TIPO_DIRECCION_PART &&
        direccion.id_pais === this.CONSTANTES.ID_COD_PAIS_SIN_RECIDENCIA
    );
    this.opcionResidenciaChile = direccionNoResidente ? false : true;
  }

  obtenerDatosDireccion(listadoDirecciones: any) {
    this.direccionParticular = listadoDirecciones.find(
      (direccion: any) =>
        direccion.id_prioridad === this.CONSTANTES.ID_TIPO_DIRECCION_PART
    );
    this.direccionComercial = listadoDirecciones.find(
      (direccion: any) =>
        direccion.id_prioridad === this.CONSTANTES.ID_TIPO_DIRECCION_COM
    );

    if (this.direccionParticular === undefined) {
      this.direccionParticular = Object.assign(
        {},
        this.CONSTANTES.OBJETO_DIRECCION_VACIO
      );
      this.direccionParticular.id_prioridad =
        this.CONSTANTES.ID_TIPO_DIRECCION_PART;
    }

    if (!this.direccionComercial === undefined) {
      this.direccionComercial = Object.assign(
        {},
        this.CONSTANTES.OBJETO_DIRECCION_VACIO
      );
      this.direccionComercial.id_prioridad =
        this.CONSTANTES.ID_TIPO_DIRECCION_COM;
    }
  }

  public obtenerDatosCargo(listadoCargos: any) {
    if (this.idCargo == -1) {
      // sin cargo seleccionada
      return;
    }

    let cargoSeleccionado = listadoCargos.find(
      (cargo: any) => cargo.id_cargo === this.idCargo
    );
  ...
  public obtenerDatosRenta(listadoRentas: any) {
    if (this.idRenta == -1) {
      // sin renta seleccionada
      return;
    }

    let rentaSeleccionada = listadoRentas.find(
      (renta: any) => renta.id === this.idRenta
    );
    this.nombreRenta = rentaSeleccionada ? rentaSeleccionada.rango : '';
  }

  public obtenerDatosProfesion(listadoProfesiones: any) {
    if (this.idProfesion == -1) {
      // sin profesion seleccionada
      return;
    }

    let profesionSeleccionada = listadoProfesiones.find(
      (prefesion: any) => prefesion.id_profesion === this.idProfesion
    );
    this.nombreProfesion = profesionSeleccionada
      ? profesionSeleccionada.nombre_profesion
      : '';
  }
  ...
  obtenerDatosEmpleador(datosUsuarioResponse: any) {
    datosUsuarioResponse.empleadores.forEach((empleador: any) => {
      ...
      empleadorGuardar.ultima_cotizacion =
        empleadorGuardar.ultima_cotizacion === ''
          ? this.CONSTANTES.SIN_FECHA_ULTIMA_COTIZACION
          : empleador.ultima_cotizacion;
