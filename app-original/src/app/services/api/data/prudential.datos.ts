...
export class PrudentialDatos {
    ...
    public obtenerProductoPrudential(detalle: CuentaPrudential[] = [], objeto: any): any {
        return {
    ...
    public obtenerValorPreferencia(verSaldos: boolean): string {
        return (verSaldos) ? CONSTANTES_PRUDENTIAL.PREFERENCIAS.VER_SALDO_S : CONSTANTES_PRUDENTIAL.PREFERENCIAS.VER_SALDO_N;
    }

    public obtenerFlagPreferencia(preferencia: string): boolean {
        return (preferencia === CONSTANTES_PRUDENTIAL.PREFERENCIAS.VER_SALDO_S) ? true : false;
    }
