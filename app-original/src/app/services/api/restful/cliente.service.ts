...
export class ClienteService {
    ...
    administrarClavePrevired(rut: number, dv: string, clave: string, transaccion: string): Observable<any> {
        ...
        let httpHeaders = new HttpHeaders({ 'codigoAFP': '1005', 'codigoUsuario': 'APP' });
        return this.http.post(url, JSON.stringify(parametros), { headers: httpHeaders });
    }

    /**
     * Servicio encargado de mezclar 2 pdf recibidos en Base64, este retorna un pdf en Base64.
     * @param pdf1 {string}
     * @param pdf2 {string}
     */
    mergePdf(pdf1: string, pdf2: string): Observable<any> {
        const url = this.dominio + '/api/v1/util/merge-pdf';
        const parametros = {
            'base64PDF1': pdf1,
            'base64PDF2': pdf2
        };
    ...
    /**
     * Servicio encargado de enviar via email el comprobante para solicitud de cambio o distribucion de fondos.
     * @param rut {number}
     * @param dv {string}
     * @param idSolicitud {number}
     */
    obtenerURLVideoCartolaMensual(rut: string, dv: string, idMaeCliente: string): Observable<any> {
        const parametros = {
            "rut_cliente":  rut,
            "dv_cliente": dv,
            "mae_cliente": idMaeCliente
        };

        const url = this.dominio + '/api/v1/clientes/' + rut + '-' + dv + '/cartolaMensual/ObtenerURLVideo';
        return this.http.post(url, JSON.stringify(parametros));
    }
