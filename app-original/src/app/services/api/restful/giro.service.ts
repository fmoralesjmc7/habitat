...
export class GiroService {
   /**
    ...
    validarGiro(producto: string, rut: number, dv: string): Observable<any> {
        const url: string = this.dominio + '/api/v1/clientes/' + rut + '-' + dv + '/giro/validacion';
        const parametros = {
            "validarGiro": {
                "producto": producto
            }
    ...
    obtenerListaTipoDeCuentas(rut: number, dv: string): Observable<any> {
        const url: string = this.dominio + '/api/v1/clientes/' + rut + '-' + dv + '/giro/tipo-cuentas';
    ...
    obtenerListaBancos(rut: number, dv: string): Observable<any> {
        const url: string = this.dominio + '/api/v1/clientes/' + rut + '-' + dv + '/giro/lista-bancos';
        return this.http.get(url);
    }
    ...
    obtenerBancosRegistrados(rut: number, dv: string) {
        const url: string = this.dominio + '/api/v1/clientes/' + rut + '-' + dv + '/giro/bancos-registrados';
        return this.http.get(url);
    }
     ...
     */
    solicitudCodigoDinamicoSMS(rut: number, dv: string, nombre: string, apellido: string, telefono: number, producto: string): Observable<any> {
    ...
    validarCodigoDinamicoSMS(rut: number, dv: string, producto: string, codigoSMS: number) {
    ...
    realizarSolicitud(solicitudGiro: SolicitudGiro): Observable<any> {
        const urlCAV: string = this.dominio + '/api/v2/clientes/' + solicitudGiro.rut + '-' + solicitudGiro.dv + '/giro/solicitud';
        const url: string = this.dominio + '/api/v1/clientes/' + solicitudGiro.rut + '-' + solicitudGiro.dv + '/giro/solicitud';
        const urlGiro = solicitudGiro.IdTipoProducto == CONSTANTES_GIRO_STEP_1.ID_PRODUCTO_CUENTA_2.toString() ? urlCAV : url; // en el caso de giro covid , apuntamos a servicio giro covid  Esto se debe eliminar una vez se utilice solo la version 2 del servicio
        const parametros = {
            "SolicitudGiro": {
    ...
    obtenerComisionGiro(paramentrosGiro: ParametrosComisionGiro){
        const url: string = this.dominio + '/api/v1/clientes/' + paramentrosGiro.rut + '-' + paramentrosGiro.dv + '/giro/buscar-comision';
        let parametros = {
     ...
     */
    validarCuentaCliente(requestCuenta: RequestValidacionCuenta): Observable<ResponseValidacionCuenta> {
        const url = `${this.dominio}/api/v1/clientes/giro/lista-negra/${requestCuenta.nrut_cuenta}`;

        return this.http.post<ResponseValidacionCuenta>(url, JSON.stringify(requestCuenta));
    }
}
