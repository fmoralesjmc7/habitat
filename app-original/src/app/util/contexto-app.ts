...
import { BarraInformativaInterface } from '../services/api/data/barra-informativa.interface';
  ...
  */
  ...
  /**
   ...
   async registrarTrazabilidad(parametroTraza: ParametroTraza) {
    ...
    this.trazabilidadService.registraTrazaUUID(parametros, this.datosCliente.rut, this.datosCliente.dv).subscribe((response) => {});
  }

  generarObjetoTraza(datosGenerales: any, traza: any) {
    const parametroTraza: ParametroTraza = new ParametroTraza();
  ...
  obtenerBarraInformativa(): Promise<BarraInformativaInterface>  {
    return new Promise((resolve, reject) => {
      this.barraInformativaService.obtenerDatosBarraInformativa().subscribe( (datos: BarraInformativaInterface) => {
        resolve(datos);
      }, error => {
        reject(undefined);
      });
